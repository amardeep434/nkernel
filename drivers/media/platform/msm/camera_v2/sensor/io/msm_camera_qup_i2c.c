	...
	struct msm_camera_i2c_reg_setting *write_setting)
{
	...
	if (write_setting->addr_type == MSM_CAMERA_I2C_WORD_ADDR &&
		...
		for (i = 0; i < num_batch; i++) {
			rc = msm_camera_qup_i2c_write_word_addr_byte_data_batch(
				client, reg_setting);
			if (rc < 0)
				break;
		...
		write_setting->data_type == MSM_CAMERA_I2C_WORD_DATA) {
		for (i = 0; i < num_batch; i++) {
			rc = msm_camera_qup_i2c_write_word_addr_word_data_batch(
				client, reg_setting);
			if (rc < 0)
				break;
			reg_setting += batch_size;
		...
		write_setting->data_type == MSM_CAMERA_I2C_BYTE_DATA) {
		for (i = 0; i < num_batch; i++) {
			rc = msm_camera_qup_i2c_write_byte_addr_byte_data_batch(
				client, reg_setting);
			if (rc < 0)
				break;
			reg_setting += batch_size;
		...
		write_setting->data_type == MSM_CAMERA_I2C_WORD_DATA) {
		for (i = 0; i < num_batch; i++) {
			rc = msm_camera_qup_i2c_write_byte_addr_word_data_batch(
				client, reg_setting);
			if (rc < 0)
				break;
			reg_setting += batch_size;
